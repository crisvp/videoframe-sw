...
let generateVideoFrame: () => Promise<VideoFrame>;
function generateVideoFrameFactory(imageEl: HTMLImageElement) {
  return async function generateVideoFrame() {
    const imageSource = imageEl;
    const videoFrame = new VideoFrame(imageSource, { timestamp: 0 });
    if (!videoFrame) throw new Error("could not create video frame");
...
async function main() {
  ...
  try {
    ...
    if (err.name !== "DataCloneError") throw new Error("unexpected error", err);
  ...
  serFrame.meta.colorSpace = JSON.parse(
    JSON.stringify(serFrame.meta.colorSpace.toJSON())
  );
...
function mainReadFrames(reader: ReadableStreamDefaultReader<VideoFrame>) {
...
async function mostlySerializableFrame(frame: VideoFrame) {
  const props = [
    "codedHeight",
    "codedRect",
    "codedWidth",
  ...
  const serializedFrame = Object.fromEntries(
    props.map((prop) => [prop, frame[prop]])
  );
...
async function handleSerializedFrameEvent(
...
async function deserializeFrame({
  buffer,
  meta,
}: {
  buffer: Uint8ClampedArray;
  ...
  try {
    ...
    if (err.name === "TypeError") console.error("Expected for Safari:", err);
  }
